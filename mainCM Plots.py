...
P = np.array([20*10**5, 25*10**5, 30*10**5, 35*10**5, 40*10**5, 45*10**5]) # Pa
...
PlotIsotermicocomponentes(X, Y, P, T, dictionary)"""

T = np.array([215, 216, 217, 218, 219, 220, 221, 222, 223, 224, 225]) # K
L = []
Ltemp = []
V = []
Vtemp = []
...
for j in range(len(T)):
    ...
    Vtemp = []
    for i in range(len(P)):
        xi, yi, l, o, Li, Vi = Flash_Algorithm(T=T[j], Tc=Tc, P=P[i], Pc=Pc, x=x, y=y, z=z, EEC='PR', w=w, K=K, R=R, kij=kij, M=M).execute()
        Ltemp.append(Li)
        Vtemp.append(Vi)
        X.append(xi)
    L.append(Ltemp)
...
PlotIsotermico(L, V, P, T, dictionary)
PlotIsotermicocomponentes(X, Y, P, T, dictionary)